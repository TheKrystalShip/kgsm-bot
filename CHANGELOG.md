...
### Changed — a removed member's departure is held for seven days (3.57.0)

Takes `TheKrystalShip.KGSM.Cluster 1.0.0-dev.21`: the bot's roster keeps a removed member's `left` row
for seven days rather than five minutes, so a member that was down when the removal was made still
learns it from this one. Takes `TheKrystalShip.KGSM.Dns 0.2.0-dev.10` too: the bot restates where it is
reached the moment its incarnation rises, so a refuted removal gives back its capability's name at
once.
...
service token, minted from the secret every member on the machine shares, and names the asking Discord
account on `X-Kgsm-Acting` as a `provider:subject` handle. The assistant resolves that person and what
they may do from its own replica of the cluster's accounts.

No tier is sent, and there is none to send. What a compromised bot could do is ask as somebody it names,
bounded by what that person actually holds — strictly narrower than a shared secret that carried an
authority along with an identity. The same is true of a confirm: the clicker is named, and the assistant
re-reads their tier at the click.

The handle is qualified with its provider, so the assistant files a person under the identity they
actually have rather than under whichever one the receiving end assumed.

A machine in no cluster mints nothing, so the conversational surface reports itself off exactly as it
does with no assistant address configured. Slash commands, announcements and the status board are
unaffected either way — they need no assistant and never did.
...
### Fixed — the packaged binary is executable again (3.47.1)
