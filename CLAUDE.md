...
### Two names: the id it is keyed by, the label it is called

A server has an **id** — generated by the engine at install, immutable, and what the guild store, the
channel bindings, the follow filter, every slash-command argument and every event payload key on —
and a **display name**, which is free text somebody chose and changes whenever they like.
`ServerLabel` is the one place the two are composed, so every surface writes them the same way round,
and `IServerLabels` looks a label up for the surfaces that hold an id and nothing else (an event names
its server by id; so does a binding). It reads the cached inventory, so a label costs no kgsm process.

- **Shown by label, keyed by id, everywhere.** The status message prints both — the label to read and
  the id to type — and so do autocomplete entries and the `/setup` listings, while the value
  autocomplete hands back is always the id. An announcement is named by label alone: the channel is
  named after the id and a player-joined line does not need it twice.
...
- **`/install`'s `name` is the display name.** The engine mints the id; a person only ever chooses
  what the server is shown as.
